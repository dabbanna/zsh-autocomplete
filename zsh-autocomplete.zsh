...
_autocomplete.main.hook() {
  ...
  if [[ -v functions[_zsh_autosuggest_highlight_reset] ]]
  then
    functions[_zsh_autosuggest_highlight_reset]=$functions[_autocomplete.no-op]
  fi
  if [[ -v functions[_zsh_autosuggest_highlight_apply] ]]
  then
    autoload +X _zsh_autosuggest_highlight_apply
    functions[_autocomplete._zsh_autosuggest_highlight_apply]=$functions[_zsh_autosuggest_highlight_apply]
    ...
    _zsh_autosuggest_widget_suggest() {
      ...
      _autocomplete._zsh_highlight
			_autocomplete._zsh_autosuggest_highlight_apply
			zle -R
			return $retval
		}
  else
    functions[_autocomplete._zsh_autosuggest_highlight_apply]=$functions[_autocomplete.no-op]
  fi
  ...
  local insert
  if zstyle -s ":autocomplete:tab:" completion insert
  then
...
_autocomplete.list-choices.hook() {
  ...
  if (( ${#BUFFER} == 0 || (PENDING + KEYS_QUEUED_COUNT) > 0 ))
  then
    return
  fi
